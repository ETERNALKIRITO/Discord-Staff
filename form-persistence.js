...
function getFormIdentifier(form) {
    // Use the same logic as getBasePath for consistency
    const path = window.location.pathname;
    if (window.location.protocol === 'file:') {
        // For local file://, just use the filename as the key base
        const filename = path.split('/').pop();
        return filename ? '/' + filename : '/unknownform'; // Ensure it starts with /
    }
    // For http/https, ensure it starts with /
